...
| Harness CI stages run on Kubernetes | The complete lower pipeline runs the Spring wrapper and gate together in an ephemeral `KubernetesDirect` pod on the customer-selected lower Kubernetes infrastructure. No infrastructure-provider connector name is committed. The drop-in stage also supports a customer-selected private namespace and ClusterIP service. |
...
| Official open-source Pact lifecycle | Pact CLI v0.10.7 is release-URL, byte-count, and SHA-256 locked. Consumer publication, provider verification, `can-i-deploy`, and `record-deployment` are separate Harness stage objects with the correct ownership boundaries. Provider evidence requires a successful, non-skipped case; deploy evidence requires a nonempty matrix with at least one success and no failed or unknown checks. |
